...
GameBoard::GameBoard(const GameBoard& src) { copyFrom(src); }
...
void GameBoard::setPieceAt(size_t x, size_t y, const GamePiece& inElem) {
  ...
  if (x >= mWidth) throw out_of_range("GameBoard::setPieceAt: x-coordinate beyond width");
  if (y >= mHeight) throw out_of_range("GameBoard::setPieceAt: y-coordinate beyond height");
...
GamePiece& GameBoard::getPieceAt(size_t x, size_t y) {
  // Check for out of range arguments
  if (x >= mWidth) throw out_of_range("GameBoard::getPieceAt: x-coordinate beyond width");
  if (y >= mHeight) throw out_of_range("GameBoard::getPieceAt: y-coordinate beyond height");
...
const GamePiece& GameBoard::getPieceAt(size_t x, size_t y) const {
  // Check for out of range arguments
  if (x >= mWidth) throw out_of_range("GameBoard::getPieceAt const: x-coordinate beyond width");
  if (y >= mHeight) throw out_of_range("GameBoard::getPieceAt const: y-coordinate beyond height");
