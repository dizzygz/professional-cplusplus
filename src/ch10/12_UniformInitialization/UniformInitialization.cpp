#include <iostream>
#include <string>
#include <vector>

using namespace std;

struct CircleStruct {
  int x, y;
  double radius;
};

class CircleClass {
public:
  CircleClass(int x, int y, double radius) : mX(x), mY(y), mRadius(radius) {}

  void print() const { cout << mX << mY << mRadius << endl; }

private:
  int mX, mY;
  double mRadius;
};

void func(int /* i */) { /* ... */
}

class MyClass {
public:
  MyClass() : mArray{0, 1, 2, 3} {}

  void print() const { cout << mArray[0] << mArray[1] << mArray[2] << mArray[3] << endl; }

private:
  int mArray[4];
};

int main() {
  // Old pre-C++11 way
  CircleStruct myCircle1 = {10, 10, 2.5};
  CircleClass myCircle2(10, 10, 2.5);

  // C++11 uniform initialization
  CircleStruct myCircle3 = {10, 10, 2.5};
  CircleClass myCircle4 = {10, 10, 2.5};
  CircleStruct myCircle5{10, 10, 2.5};
  CircleClass myCircle6{10, 10, 2.5};

  int a = 3;
  int b(3);
  int c = {3};  // Uniform initialization
  int d{3};     // Uniform initialization

  // Narrowing (Warning on most compilers)
  // int x = 3.14;
  // func(3.14);

  // Preventing narrowing (Error on most compilers)
  // int x = {3.14};    // Error or warning because narrowing
  // func({3.14});      // Error or warning because narrowing

  // Uniform initialization also works with vectors and maps
  vector<string> myVec = {"String 1", "String 2", "String 3"};

  // Uniform initialization can also be used with dynamically allocated arrays
  int* pArray = new int[4]{0, 1, 2, 3};

  // Create a MyClass object
  MyClass c;

  return 0;
}
