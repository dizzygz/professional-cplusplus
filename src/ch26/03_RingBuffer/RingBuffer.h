...
// Provides a simple debug rign buffer. The client specifies the number of entries in the constructor and adds
// messages with the addEntry() method. Once the number of entries exceeds the number allowed, new entries
// overwrite the oldest entries in the buffer.
//
// The buffer also provides the option to output entries to a stream as they are added to the buffer. The
// client can specify an output stream in the constructor, and can reset it with the setOutput() method.
//
...
public:
  ...
  // Adds the string to the ring buffer, possibly overwriting the oldest string in the buffer (if the buffer
  // is full).
