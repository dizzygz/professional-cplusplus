...
// Initialize the vector to hold exactly numEntries. The vector size does not need to change during the
// lifetime of the object. Initialize the other members too.
RingBuffer::RingBuffer(size_t numEntries, ostream* ostr)
    : mEntries(numEntries), mNext(begin(mEntries)), mOstr(ostr), mWrapped(false) {}

RingBuffer::~RingBuffer() {}

// The addEntry algorithm is pretty simple: add the entry to the next free spot, then reset mNext to indicate
// the free spot after that. If mNext reaches the end of the vector, it starts over at 0.
//
// The buffer needs to know if the buffer has wrapped or not so that it knows whether to print the entries
// past mNext in operator<<
void RingBuffer::addStringEntry(string&& entry) {
  ...
  // Move the entry to the next free spot and increment mNext to point to the free spot after that.
