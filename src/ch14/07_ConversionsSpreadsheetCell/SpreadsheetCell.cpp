...
SpreadsheetCell::SpreadsheetCell(double initialValue) { set(initialValue); }
...
SpreadsheetCell operator/(const SpreadsheetCell& lhs, const SpreadsheetCell& rhs) {
  if (rhs.mValue == 0) throw invalid_argument("Divide by zero.");
  SpreadsheetCell newCell;
  newCell.set(lhs.mValue / rhs.mValue);  // call set to update mValue and mString
  return newCell;
}
...
bool operator==(const SpreadsheetCell& lhs, const SpreadsheetCell& rhs) { return lhs.mValue == rhs.mValue; }
...
SpreadsheetCell::operator string() const { return mString; }
