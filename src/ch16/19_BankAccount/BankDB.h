...
class BankDB {
public:
  // Adds acct to the bank database. If an account exists already with that number, the new account is not
  // added. Returns true if the account is added, false if it's not.
  bool addAccount(const BankAccount& acct);

  // Removes the account acctNum from the database.
  void deleteAccount(int acctNum);

  // Returns a reference to the account represented by its number or the client name. Throws out_of_range if
