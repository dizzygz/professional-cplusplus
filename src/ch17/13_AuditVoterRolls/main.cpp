...
// Returns a set of all names that appear in more than one list in the map.
//
// The implementation generates one large list of all the names from all the lists in the map, sorts it, then
// finds all duplicates in the sorted list with adjacent_find().
set<string> getDuplicates(const VotersMap& votersByDistrict) {
  // Collect all the names from all the lists into one big list.
  list<string> allNames;
  for (const auto& district : votersByDistrict) {
  ...
  // Now it's sorted, all duplicate names will be next to each other. Use adjacent_find() to find instances of
  // two or more identical names next to each other. Loop until adjacent_find() returns the end iterator.
...
// Expects a map of string/list<string> pairs keyed on district names and containing lists of all the
// registered voters in those districts.
//
// Removes from each list any name on the convictedFelons list and any name that is found on any other list.
void auditVoterRolls(VotersMap& votersByDistrict, const list<string>& convictedFelons) {
  ...
  // Combine the duplicates and convicted felons -- we want to remove names on both lists from all voter
  // rools.
  ...
  for_each(begin(votersByDistrict), end(votersByDistrict), [&toRemove](DistrictPair& district) {
...
int main() {
  ...
  VotersMap voters = {{"Orange", {"Amy Aardvark", "Bob Buffalo", "Charles Cat", "Dwayne Dog"}},
